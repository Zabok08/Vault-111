...
// @description  Vault 111 scheduling, notifications, dashboard, OC planning, war tracking, payouts, and member analytics.
// @author       Vault 111
...
(() => {
  ...
  instanceMarker.dataset.version = '3.5.0-alpha.1';
  ...
  const TAB_IDS = ['dashboard', 'plan', 'members', 'war', 'payouts', 'schedule', 'backend', 'settings'];
    ...
    root.innerHTML = `
      <header data-drag-handle${state.settings.collapsed ? ' tabindex="0" aria-label="Collapsed planner. Drag or use arrow keys to move."' : ''}>
        <div>
          ...
          <small>v3.5 alpha.1 · ${state.backend.connected ? '<b class="backend-label">BACKEND CONNECTED</b> · ' : ''}${syncedAt ? `Synced ${new Date(syncedAt).toLocaleString()}` : 'Not synced'}</small>
        </div>
