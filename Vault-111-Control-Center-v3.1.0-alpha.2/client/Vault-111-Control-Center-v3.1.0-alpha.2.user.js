...
// @description  Vault 111 OC planning, ranked-war tracking, shared target lists, and officer notes.
// @author       Vault 111
...
(() => {
  ...
  instanceMarker.dataset.version = '3.1.0-alpha.2';
  ...
  const TAB_IDS = ['dashboard', 'plan', 'members', 'war', 'backend', 'settings'];
    ...
    root.innerHTML = `
      <header data-drag-handle${state.settings.collapsed ? ' tabindex="0" aria-label="Collapsed planner. Drag or use arrow keys to move."' : ''}>
        <div>
          ...
          <small>v3.1 alpha.2 · ${state.backend.connected ? '<b class="backend-label">BACKEND CONNECTED</b> · ' : ''}${syncedAt ? `Synced ${new Date(syncedAt).toLocaleString()}` : 'Not synced'}</small>
        </div>
  ...
  async function synchronizeWarTracker() {
    ...
    try {
      ...
      const result = await loadWarSnapshot();
      const message = result?.war
        ? `Ranked War #${result.war.id} synchronized with ${formatNumber(sync?.targets ?? result?.targets?.length)} targets and ${formatNumber(sync?.attacks)} attacks.`
