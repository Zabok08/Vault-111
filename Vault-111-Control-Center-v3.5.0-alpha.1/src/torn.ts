...
const battleStatDetailSchema = z.object({
  value: analyticsCountSchema
}).passthrough();
const battleStatsResponseSchema = z.object({
  battlestats: z.object({
    strength: battleStatDetailSchema,
    defense: battleStatDetailSchema,
    speed: battleStatDetailSchema,
    dexterity: battleStatDetailSchema,
    total: analyticsCountSchema
  }).passthrough()
}).passthrough();
const drugStatsResponseSchema = z.object({
  personalstats: z.object({
    drugs: z.object({
      cannabis: analyticsCountSchema,
      ecstasy: analyticsCountSchema,
      ketamine: analyticsCountSchema,
      lsd: analyticsCountSchema,
      opium: analyticsCountSchema,
      pcp: analyticsCountSchema,
      shrooms: analyticsCountSchema,
      speed: analyticsCountSchema,
      vicodin: analyticsCountSchema,
      xanax: analyticsCountSchema,
      total: analyticsCountSchema,
      overdoses: analyticsCountSchema,
      rehabilitations: z.object({
        amount: analyticsCountSchema,
        fees: analyticsCountSchema
      }).passthrough()
    }).passthrough()
  }).passthrough()
}).passthrough();
const cooldownsResponseSchema = z.object({
  cooldowns: z.object({
    drug: analyticsCountSchema,
    medical: analyticsCountSchema,
    booster: analyticsCountSchema
  }).passthrough()
}).passthrough();
...
export function parseBattleStats(value: unknown) {
  const stats = battleStatsResponseSchema.parse(value).battlestats;
  return {
...
export function parseDrugStats(value: unknown) {
  const drugs = drugStatsResponseSchema.parse(value).personalstats.drugs;
  return {
    cannabis: drugs.cannabis,
    ecstasy: drugs.ecstasy,
    ...
    speed: drugs.speed,
    vicodin: drugs.vicodin,
    xanax: drugs.xanax,
    total: drugs.total,
    overdoses: drugs.overdoses,
    rehabilitationCount: drugs.rehabilitations.amount,
    rehabilitationFees: drugs.rehabilitations.fees
  };
}

export function parseCooldowns(value: unknown) {
  const cooldowns = cooldownsResponseSchema.parse(value).cooldowns;
  return {
    drug: cooldowns.drug,
    medical: cooldowns.medical,
    booster: cooldowns.booster
  };
}
...
export async function fetchMemberAnalytics(apiKey: string) {
  const settled = await Promise.allSettled([
    tornGet<unknown>("/user/battlestats", apiKey),
    tornGet<unknown>("/user/personalstats?cat=drugs", apiKey),
    tornGet<unknown>("/user/cooldowns", apiKey)
  ]);
  const labels = ["Battle stats", "Drug totals", "Cooldowns"];
  const values: [ReturnType<typeof parseBattleStats> | null, ReturnType<typeof parseDrugStats> | null, ReturnType<typeof parseCooldowns> | null] = [null, null, null];
  const warnings: string[] = [];
  let firstOperationalError: unknown = null;

  settled.forEach((result, index) => {
    if (result.status === "rejected") {
      const error = result.reason;
      if (error instanceof TornApiError && [401, 429].includes(error.statusCode)) {
        firstOperationalError ??= error;
      } else if (error instanceof TornApiError && error.statusCode === 403) {
        warnings.push(`${labels[index]} permission is missing from this API key.`);
      } else {
        firstOperationalError ??= error;
        warnings.push(`${labels[index]} could not be synchronized.`);
      }
      return;
    }
    try {
      if (index === 0) values[0] = parseBattleStats(result.value);
      else if (index === 1) values[1] = parseDrugStats(result.value);
      else values[2] = parseCooldowns(result.value);
    } catch (error) {
      firstOperationalError ??= error;
      warnings.push(`${labels[index]} was returned in an unsupported format.`);
    }
  });

  if (!values.some(Boolean)) {
    if (firstOperationalError instanceof TornApiError) throw firstOperationalError;
    if (firstOperationalError instanceof z.ZodError) {
      throw new TornApiError("Torn API returned an unsupported member-analytics format", 502);
    }
    throw new TornApiError(
      "Your API key needs the user:battlestats, user:personalstats, and user:cooldowns selections",
      403
    );
  }
