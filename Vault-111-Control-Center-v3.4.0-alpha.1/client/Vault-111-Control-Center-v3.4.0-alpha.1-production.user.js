...
// @description  Vault 111 faction dashboard, announcements, OC planning, war tracking, payouts, and member analytics.
// @author       Vault 111
...
(() => {
  ...
  instanceMarker.dataset.version = '3.4.0-alpha.1';
    ...
    root.innerHTML = `
      <header data-drag-handle${state.settings.collapsed ? ' tabindex="0" aria-label="Collapsed planner. Drag or use arrow keys to move."' : ''}>
        <div>
          ...
          <small>v3.4 alpha.1 · ${state.backend.connected ? '<b class="backend-label">BACKEND CONNECTED</b> · ' : ''}${syncedAt ? `Synced ${new Date(syncedAt).toLocaleString()}` : 'Not synced'}</small>
        </div>
