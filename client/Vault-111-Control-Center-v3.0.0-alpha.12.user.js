...
(() => {
  ...
  instanceMarker.dataset.version = '3.0.0-alpha.12';
    ...
    root.innerHTML = `
      <header data-drag-handle${state.settings.collapsed ? ' tabindex="0" aria-label="Collapsed planner. Drag or use arrow keys to move."' : ''}>
        <div>
          ...
          <small>v3.0 alpha.12 · ${state.backend.connected ? '<b class="backend-label">BACKEND CONNECTED</b> · ' : ''}${syncedAt ? `Synced ${new Date(syncedAt).toLocaleString()}` : 'Not synced'}</small>
        </div>
  ...
  function addStyles() {
    GM_addStyle(`
      ...
      @media(max-width:600px) {
        ...
        #v111-ocp:not(.collapsed) .body { max-height:calc(40vh - 44px) !important; }
        #v111-ocp.collapsed { width:min(260px,calc(100vw - 24px)) !important; max-height:48px !important; }
        #v111-ocp header { min-height:44px !important; padding:8px 10px !important; }
        #v111-ocp main { padding:8px !important; }
        #v111-ocp .tabs button { flex-basis:64px !important; }
      ...
      @media(max-width:600px) {
        ...
        #v111-ocp .member-modal { right:6vw !important; width:min(88vw,360px) !important; max-height:40vh !important; }
      }
