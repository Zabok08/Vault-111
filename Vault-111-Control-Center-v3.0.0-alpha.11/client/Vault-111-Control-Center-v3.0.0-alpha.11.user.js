...
(() => {
  ...
  let state = {
    ...
    settings: load(STORE.settings, { collapsed: false, planningOpen: true, showBreakdown: true, filter: 'all', autoRefresh: false, refreshMinutes: 5, compact: false }),
  ...
  state.settings = Object.assign({ collapsed:false, planningOpen:true, showBreakdown:true, filter:'all', autoRefresh:false, refreshMinutes:5, compact:false }, state.settings || {});
  ...
  function render() {
    ...
    const previousBody = root.querySelector('#v111-body');
    if (renderedTab && previousBody) state.ui.scrollByTab[renderedTab] = previousBody.scrollTop;
    ...
    root.innerHTML = `
      <header>
        <div>
          ...
          <small>v3.0 alpha.11 · ${state.backend.connected ? '<b class="backend-label">BACKEND CONNECTED</b> · ' : ''}${syncedAt ? `Synced ${new Date(syncedAt).toLocaleString()}` : 'Not synced'}</small>
        </div>
    ...
    requestAnimationFrame(() => {
      const body = root?.querySelector('#v111-body');
      if (body) body.scrollTop = Number(state.ui.scrollByTab[state.ui.activeTab] || 0);
  ...
  function renderBackendPanel() {
    ...
    return `
      <div class="backend-card">
        ...
        <p class="notice" id="v111-key-help">Enter only your own Torn API key. It is sent to the configured Vault 111 backend for identity and faction verification, then stored encrypted by the server. Connecting also imports only your crime-category stats for shared planner scoring. The userscript does not store the API key locally.</p>
  ...
  function activateTab(tab, focusTab = false) {
    ...
    const body = root.querySelector('#v111-body');
    if (body) state.ui.scrollByTab[state.ui.activeTab] = body.scrollTop;
    ...
    if (body) body.scrollTop = Number(state.ui.scrollByTab[tab] || 0);
  ...
  async function syncBackendPersonalStats() {
    if (state.backend.loading) return;
    state.backend.error = '';
    state.ui.backendStatus = null;
    ...
    try {
      ...
      await loadSharedPlan();
      state.backend.error = '';
      setFeedback('backend', 'Your crime stats were synchronized successfully.');
    } catch (error) {
      state.backend.error = friendly(error);
      setFeedback('backend', state.backend.error, true);
    } finally {
  ...
  function addStyles() {
    GM_addStyle(`
      ...
      #v111-ocp .body { display:block !important; max-height:calc(82vh - 48px) !important; overflow:auto !important; scrollbar-width:thin; scrollbar-color:#56748f #111820; }
      #v111-ocp.collapsed .body { display:none !important; }
      #v111-ocp.collapsed { width:300px !important; }
      #v111-ocp .tabs { position:sticky !important; top:0 !important; z-index:10 !important; display:flex !important; margin:0 !important; overflow-x:auto !important; scrollbar-width:thin !important; background:#0d131a !important; border-bottom:1px solid #26384a !important; }
      ...
      #v111-ocp .activity-bar { display:flex !important; align-items:center !important; gap:8px !important; padding:8px 12px !important; background:#1a2c3b !important; border-bottom:1px solid #36536b !important; color:#eef7ff !important; font-weight:700 !important; }
      ...
      #v111-ocp main { display:block !important; padding:12px !important; margin:0 !important; }
      ...
      @media(max-width:600px) {
        #v111-ocp { right:6px !important; top:50% !important; bottom:auto !important; transform:translateY(-50%) !important; width:calc(100vw - 12px) !important; max-height:88vh !important; }
        #v111-ocp .body { max-height:calc(88vh - 48px) !important; }
        ...
        #v111-ocp .crime-title a.button, #v111-ocp .toolbar > * { width:100% !important; }
