# Vault 111 Control Center — Version 3.1 alpha.1
...
- A fixed tab-navigation layer with independently scrolling content, plus denser mobile cards and controls
...
7. Install `client/Vault-111-Control-Center-v3.1.0-alpha.1.user.js` in Tampermonkey.
...
This alpha synchronizes faction membership, currently available Recruiting/Planning crimes, the current or most recent ranked war, outgoing ranked-war attacks, and each consenting connected member's own Torn crime-category stats. Members must connect and sync individually; the backend does not impersonate members or bulk-collect private data through another member's key. It does not yet schedule unattended syncs, calculate war payouts, run the optimizer server-side, expose role-management UI, or provide real-time updates. The Version 2 optimizer still runs inside Tampermonkey using the shared normalized stats.
