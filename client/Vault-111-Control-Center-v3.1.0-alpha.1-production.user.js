...
(() => {
  ...
  instanceMarker.dataset.version = '3.1.0-alpha.1';
    ...
    root.innerHTML = `
      <header data-drag-handle${state.settings.collapsed ? ' tabindex="0" aria-label="Collapsed planner. Drag or use arrow keys to move."' : ''}>
        <div>
          ...
          <small>v3.1 alpha.1 · ${state.backend.connected ? '<b class="backend-label">BACKEND CONNECTED</b> · ' : ''}${syncedAt ? `Synced ${new Date(syncedAt).toLocaleString()}` : 'Not synced'}</small>
        </div>
  ...
  async function synchronizeWarTracker() {
    ...
    state.ui.warStatus = null;
    if (!beginBackendWork('Synchronizing ranked-war scores and attacks from Torn…', 'war')) return;
    try {
      const sync = await backendApi('POST', '/v1/war/sync', {}, { timeoutMs: 45000 });
      const result = await loadWarSnapshot();
      const message = result?.war
        ? `Ranked War #${result.war.id} synchronized with ${formatNumber(sync?.attacks)} attacks.`
