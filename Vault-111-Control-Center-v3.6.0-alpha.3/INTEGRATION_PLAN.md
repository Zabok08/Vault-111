...
## Phase 3.5 — scheduler and notifications

- Add faction-scoped manual Chain, Ranked War, OC, Faction, Meeting, and Other events. (Implemented.)
- Derive read-only upcoming Ranked War and OC-ready events from synchronized backend records. (Implemented.)
- Enforce full, war-only, OC-only, and read-only schedule permissions on the backend. (Implemented.)
- Add optimistic event version checks, validation, rate limits, retention cleanup, and audit events. (Implemented.)
- Add self-only notification preferences for event types, browser notifications, and up to five reminder times. (Implemented.)
- Deliver deduplicated in-app and optional Tampermonkey notifications while Torn is open. (Implemented.)
- Add upcoming scheduled events and countdowns to the unified Dashboard. (Implemented.)
- Defer reliable offline push and unattended Torn synchronization to a future worker-based deployment.

Exit criteria: every role can read one shared schedule, unauthorized event types return `403`, concurrent changes return `409`, automatic events are not editable, reminder settings are per-member, and no reminder exposes credentials.

## Phase 3.6 — administration and permissions

- Add a protected Administration tab for Owner and Administrator roles. (Implemented.)
- Show faction-position mappings, connected-user access state, safe API connection status, active-session counts, and synchronization health. (Implemented.)
- Give Administrators read-only access while reserving all permission and account mutations for the Owner. (Implemented.)
- Add optimistic, Owner-only mapping create/update/delete controls that cannot assign Owner. (Implemented.)
- Apply mapping changes to existing non-Owner users and revoke affected sessions immediately. (Implemented.)
- Add Owner-only user suspension/restoration and explicit session revocation. (Implemented.)
- Add server-checked session versions so existing access tokens are invalidated immediately. (Implemented.)
- Scope audit reads to the authenticated faction and omit IP hashes, token material, and API-key data. (Implemented.)
- Flag users absent from the latest faction sync for manual review; defer automatic suspension to scheduled membership verification.

Exit criteria: Administrators cannot mutate access, Members receive `403`, Owner accounts cannot be changed from the UI, mapping conflicts return `409`, affected sessions stop working, and no administration response exposes credentials.
