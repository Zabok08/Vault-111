...
Install the resulting `client/Vault-111-Control-Center-v3.6.0-alpha.1-production.user.js`. The generator adds only the exact backend hostname to `@connect`; it does not use a wildcard or include secrets.
...
10. Verify Administrators can read—but cannot change—the Admin tab and that only the Owner can change mappings or member access.
