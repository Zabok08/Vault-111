...
const permissions: Record<AppRole, Set<string>> = {
  OWNER: new Set(["*"]),
  ADMIN: new Set(["oc.read", "oc.sync", "oc.assign", "oc.optimize", "roles.read", "audit.read"]),
  OC_PLANNER: new Set(["oc.read", "oc.sync", "oc.assign", "oc.optimize"]),
  WAR_MANAGER: new Set(["oc.read"]),
  OFFICER: new Set(["oc.read"]),
  MEMBER: new Set(["oc.read"])
};
